...
from models import RegisterRequest, LoginRequest, MedicalRecord, SearchQuery, AskQuery
...
def check_limit(db: Session, tenant: Tenant, action: str):
    ...
    if action == ACTION_FILE:
        ...
        if used >= limits["files_per_month"]:
            raise HTTPException(status_code=402, detail="Monthly file limit reached")

    if action == ACTION_RECORD:
        used = get_monthly_usage(db, tenant.id, ACTION_RECORD)
        if used >= limits["manual_records_per_month"]:
            raise HTTPException(status_code=402, detail="Monthly manual record limit reached")


def activate_tenant_pro(db: Session, tenant: Tenant, subscription_id: str | None = None):
    tenant.plan = "pro"
...
def billing_checkout(user: User = Depends(require_admin), db: Session = Depends(get_db)):
    tenant = db.query(Tenant).filter(Tenant.id == user.tenant_id).first()
...
async def lemon_webhook(request: Request, db: Session = Depends(get_db)):
    if not LEMON_WEBHOOK_SECRET:
        raise HTTPException(status_code=500, detail="Lemon webhook secret is not configured")
    ...
    if event_name in activate_events:
        if tenant:
            tenant = activate_tenant_pro(db, tenant, subscription_id)
            return {
                "received": True,
                "updated": True,
...
@app.post("/search")
def search(query: SearchQuery, user: User = Depends(get_current_user)):
    results = query_rag(
...
async def upload_csv(
    ...
    if "clinical_note" not in df.columns:
        raise HTTPException(status_code=400, detail="CSV must contain clinical_note column")
